...
<RequestExample>
```bash
curl -X GET "https://dev.kyc.legaltalent.ai/exports/abc-123-def" \
...
```json
{
  ...
  "presigned_url": "https://kyc-exports-develop.s3.amazonaws.com/exports/tenants/tenant123/abc-123-def.pdf?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...&X-Amz-Date=20241122T103000Z&X-Amz-Expires=3600&X-Amz-Signature=..."
}
```
</ResponseExample>
...
```bash
# Get presigned URL
RESPONSE=$(curl -X GET "https://dev.kyc.legaltalent.ai/exports/abc-123-def" \
  -H "Authorization: Bearer YOUR_TOKEN")
...
response = requests.get(
    f"https://dev.kyc.legaltalent.ai/exports/{job_id}",
...
async function downloadReport() {
  ...
  const response = await fetch(
    `https://dev.kyc.legaltalent.ai/exports/${jobId}`,
    {
      method: "GET",
      headers: {
